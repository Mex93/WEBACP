...
def get_pallet_info_sn_data(pallet_sn: str):
    response_for_client = {
        "error_text": "",
        "result": False
    }
    account_idx = -1
    account_name = 'Пользователь без права авторизации'
    count = 0
    ...
    try:
        ...
        if result_connect is True:
                    ...
                    if data:
                        for device in data:
                            ...
                            if None not in (item_device_assy, item_device_sn):
                                ...
                                if not model_get_count:
                                    model_get_count = 1

                                    this_model_name = csql.get_tv_model_name(item_model_fk)
                                    if isinstance(this_model_name, str):
            ...
            if pallet_scanned_result:
                ...
                response_for_client.update({"pallet_number": pallet_number})
                ...
                if count:
                    ...
                    response_for_client.update({"pallet_device_model_name": model_name})
                    response_for_client.update({"pallet_device_fk": model_fk})
                    cdebug.debug_print(
                        f"get_pallet_info_sn_data AJAX -> [Получение информации о паллете] -> [IDX:{account_idx}, {account_name}] -> "
        ...
        response_for_client.update({"error_text": "errorcode: get_pallet_info_sn_data -> [NotConnectToDB]"})
        cdebug.debug_print(
            f"get_pallet_info_sn_data AJAX -> [Получение информации о паллете '{pallet_sn}'] -> [IDX:{account_idx}, {account_name}] -> "
            f"[Исключение] [NotConnectToDB: '{err}']")

    except ErrorSQLQuery as err:
        response_for_client.update({"error_text": "errorcode: get_pallet_info_sn_data -> [ErrorSQLQuery]"})
        cdebug.debug_print(
            f"get_pallet_info_sn_data AJAX -> [Получение информации о паллете '{pallet_sn}'] -> [IDX:{account_idx}, {account_name}] -> "
            f"[Исключение] [ErrorSQLQuery: '{err}']")

    except ErrorSQLData as err:
        response_for_client.update({"error_text": "errorcode: get_pallet_info_sn_data -> [ErrorSQLData]"})
        cdebug.debug_print(
            f"get_pallet_info_sn_data AJAX -> [Получение информации о паллете '{pallet_sn}'] -> [IDX:{account_idx}, {account_name}] -> "
        ...
        response_for_client.update({"error_text": "errorcode: get_pallet_info_sn_data -> [Error Data]"})
        cdebug.debug_print(
            f"get_pallet_info_sn_data AJAX -> [Получение информации о паллете '{pallet_sn}'] -> [IDX:{account_idx}, {account_name}] -> "
