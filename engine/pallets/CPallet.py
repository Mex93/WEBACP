...
class CPallet:
    __cparams = [
        ['pallet_sn', 'Номер паллета', SQL_PALLET_SN_FIELDS.fd_pallet_code, str, EDIT_FIELD.NO_EDIT, INPUT_TYPE.INPUT],
        ['assy_id', 'Номер паллета в БД', SQL_PALLET_SN_FIELDS.fd_assy_id, int, EDIT_FIELD.NO_EDIT, INPUT_TYPE.INPUT],
        ['completed_check', 'Статус', SQL_PALLET_SN_FIELDS.fd_completed_check, bool, EDIT_FIELD.EDITED, INPUT_TYPE.CHECKBOX],
        ['assembled_line', 'Сборочная линия', SQL_PALLET_SN_FIELDS.fd_assembled_line, int, EDIT_FIELD.EDITED, INPUT_TYPE.INPUT],
        ['create_date', 'Дата создания', SQL_PALLET_SN_FIELDS.fd_created_data, str, EDIT_FIELD.NO_EDIT, INPUT_TYPE.INPUT],
        ['completed_date', 'Дата закрытия', SQL_PALLET_SN_FIELDS.fd_completed_date, str, EDIT_FIELD.NO_EDIT, INPUT_TYPE.INPUT],
    ]
    ...
    @classmethod
    def get_value_type(cls, array_index: int) -> any:
        if 0 <= array_index <= len(cls.__cparams):
